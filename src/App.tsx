...
function App() {
	...
	const onDragEnd = (result: DropResult) => {
		...
		if (source.droppableId === "Maker") {
			if (staged) {
				...
				const index = TriggerLists.findIndex(
					(element) => element.id == destination.droppableId
				);

				if (
					staged &&
					destination.droppableId == "onAwaken" &&
					["move", "rot", "handud", "handoc"].includes(staged.component)
				) {
					setAwakeCond(true);
					return;
				} else if (
					staged &&
					!["onAwaken", "Maker"].includes(destination.droppableId) &&
					staged.component == "wait"
				) {
					setWaitCond(true);
					return;
				}
		...
		if (source.droppableId === destination?.droppableId) {
			...
			const sourceIndex = TriggerLists.findIndex(
				(element) => element.id == source.droppableId
			);
			const destIndex = TriggerLists.findIndex(
				(element) => element.id == destination.droppableId
			);
			...
			if (
				_insertdata &&
				destination.droppableId == "onAwaken" &&
				["move", "rot", "handud", "handoc"].includes(_insertdata.component)
			) {
				setAwakeCond(true);
				return;
			} else if (
	...
	const TryLogin = () => {
		...
		signInWithEmailAndPassword(auth, userID + "@example.com", password)
			.then(() => {
				setStepIndex(1);
				setUName(userID);
	...
	async function UploadData(userId: string, userData: (BlockType | null)[][]) {
		try {
			const uploadBlocks = {
				...
				onRightTrigger: userData[5],
				onLeftTrigger: userData[6],
				onRButton: userData[7],
				onLButton: userData[8],
			};
				...
				>
					...
					<Stack gap={3} sx={{ m: 3, flexGrow: 1 }}>
						<Box>
							...
							<Typography sx={{ fontSize: 10, color: "#909090" }}>
								ここにコードを書いてください
							</Typography>
						</Box>
							...
							<Stack gap={3}>
								...
								{TriggerLists.map((Trigger, index) => (
									<Paper elevation={1} sx={{ p: 2 }}>
										...
										<Typography
											variant='body2'
											sx={{ color: "#909090", fontSize: "12px" }}
										>
											{Trigger.AlternativeName}
										</Typography>
						...
						<Box sx={{ p: 1, flexGrow: 1, overflowY: "scroll" }}>
							<Paper elevation={3} sx={{ p: 3, ml: 1 }}>
								...
								>
									<AddbuttonListItem
										Name='ロボットを移動させる'
										NameAlternative='ロボットをいどうさせる'
										icon={<ControlCameraIcon />}
										onclick={() =>
											setStaged({ id: uuid(), value: "", component: "move" })
										}
									/>
									<AddbuttonListItem
										Name='ロボットを回転させる'
										NameAlternative='ロボットをまわす'
										icon={<ThreeSixtyIcon />}
										onclick={() =>
											setStaged({ id: uuid(), value: "", component: "rot" })
										}
									/>
								</AddButton>
                  ...
                  <AddbuttonListItem Name='ハンドを開閉させる' NameAlternative='ハンドをひらく・とじる' icon={<SyncAltIcon/>} onclick={ ()=>setStaged({id: uuid(), value: "", component: 'handoc'})}/>
								...
								>
									<AddbuttonListItem
										Name='赤色のLEDを光らせる・消す'
										NameAlternative='あかいろのLEDをひからせる・けす'
										icon={<LightModeIcon sx={{ color: "#F06060" }} />}
										onclick={() =>
											setStaged({ id: uuid(), value: "", component: "r_led" })
										}
									/>
									<AddbuttonListItem
										Name='緑色のLEDを光らせる・消す'
										NameAlternative='みどりいろのLEDをひからせる・けす'
										icon={<LightModeIcon sx={{ color: "#60F060" }} />}
										onclick={() =>
											setStaged({ id: uuid(), value: "", component: "g_led" })
										}
									/>
								...
								<AddButton
									name='時間を待つ'
									sx={{ backgroundColor: "#E0E0FF" }}
								>
									<AddbuttonListItem
										Name='待機する'
										NameAlternative='じかんをまつ'
										icon={<AccessAlarmIcon />}
										onclick={() =>
											setStaged({ id: uuid(), value: "", component: "wait" })
										}
									/>
								</AddButton>
							</Paper>
											...
											>
												...
												<Box gap={1} sx={{ display: "flex" }}>
													...
													<Button
														variant='outlined'
														disabled={duration}
														onClick={() => UploadData(uName, blocks)}
													>
														アップロード
													</Button>
												</Box>
